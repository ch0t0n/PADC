...
class MultiTaskPartModel(L.LightningModule):
    ...
    def _compute_metrics(self, mask_logits, mask, species_logits, species_label, stage):
        ...
        # 2. Segmentation IoU (Macro) - Manual computation to avoid backend issues
        preds = mask_logits.argmax(dim=1)
        ious = []
        for cls in range(self.hparams.num_parts):
            pred_inds = (preds == cls)
            target_inds = (mask == cls)
            intersection = (pred_inds & target_inds).sum().float()  # Sum over all dims
            union = (pred_inds | target_inds).sum().float()
            if union == 0:
                ious.append(1.0)  # Treat as perfect match if no ground truth or pred for class
            else:
        ...
        self.log(f"{stage}_acc", acc,  on_step=False, on_epoch=True, prog_bar=True, logger=True, sync_dist=True)
...
BATCH_SIZE = 64
IMAGE_SIZE = 320
DEVICE_IDS = [4,5,6,7]  # List of GPU IDs
